...
export default function CameraScreen({ stripType, onComplete, onBack }: Props) {
  ...
  const captureFrame = useCallback((snapshotFilter: FilterId): string | null => {
    ...
    if (css !== 'none') {
      ...
      return off.toDataURL('image/jpeg', 0.93)
    }
  ...
  const startSession = useCallback(async () => {
    ...
    for (let shot = 0; shot < needed; shot++) {
      ...
      await sleep(100)
      setFlash(false)
      const dataUrl = captureFrame(filterRef.current)
      if (dataUrl) {
  ...
  return (
    <div className="bg-lumi min-h-screen flex flex-col">
      ...
      <div className="flex flex-col lg:flex-row gap-5 px-4 sm:px-6 pb-8 flex-1 max-w-[1100px] mx-auto w-full">
            ...
            <div className="p-4">
              <button
                ...
                {shooting
                  ...
                  : <><span style={{ fontSize: 20 }}>📸</span> Start Shooting</>
                }
              </button>
            </div>
          </div>
        ...
        <div className="lg:w-64 flex flex-col gap-4">
          <div className="card p-4 anim-fade" style={{ animationDelay: '0.2s' }}>
            ...
            <p className="text-[10px] text-center mt-3" style={{ color: 'var(--muted)', letterSpacing: '0.05em' }}>
              {photos.length === 0 ? 'Your photos will appear here' : photos.length === needed ? 'All shots captured! ✨' : `${needed - photos.length} more to go`}
            </p>
          </div>
        </div>
      </div>
